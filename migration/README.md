...
java -Dflyway.skipCheckForUpdate=true \
  ...
  -configFiles=migration/config/dev/flyway.toml \
  migrate
```

接続情報と環境別のマイグレーションパスは `config/<環境名>/flyway.toml` から読み込みます。
実行時に `DATASOURCE_URL`、`DATASOURCE_USERNAME`、`DATASOURCE_PASSWORD`、`ENV` を環境変数として設定してください。
